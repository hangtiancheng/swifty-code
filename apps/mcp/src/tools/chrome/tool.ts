...
import type { SwiftyForChromeContext, Logger as ChromeLogger, SocketClient } from "./types.js";

const SERVER_NAME = "Swifty Chrome";
...
const context: SwiftyForChromeContext = {
  serverName: SERVER_NAME,
  logger: chromeLogger,
  socketPath:
    process.platform === "win32" ? `\\\\.\\pipe\\${SOCKET_NAME}` : join(tmpdir(), SOCKET_NAME),
  getSocketPaths: getChromeSocketPaths,
  clientTypeId: "claude-code",
  onToolCallDisconnected: () =>
    "Chrome browser extension is not connected. Ensure the Swifty browser extension is installed, enabled, and running, then retry.",
};
