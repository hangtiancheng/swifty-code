...
import type { Logger, SwiftyForChromeContext } from "@/tools/chrome/types.js";
...
function createContext(socketPath: string): SwiftyForChromeContext {
  return {
...
describe("McpSocketClient reconnection", () => {
  ...
  it("reconnects the same client after the native host restarts", async () => {
    dir = mkdtempSync(join(tmpdir(), "swifty-mcp-socket-test-"));
