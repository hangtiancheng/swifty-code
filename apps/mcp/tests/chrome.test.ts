...
import type { SwiftyForChromeContext, Logger, SocketClient } from "@/tools/chrome/types.js";
...
function createContext(isDisabled = false): SwiftyForChromeContext {
  return {
...
describe("Chrome tools", () => {
  ...
  it("does not register tools when disabled", async () => {
    ...
    const server = new McpServer({ name: "test-server", version: "0.0.0" });
    server.registerTool("sentinel", { inputSchema: z.object({}) }, async () => ({
