...
export type AcpRuntimeFactory = (workDir: string, sessionId?: string) => Promise<AcpRuntime>;
...
export interface SwiftyAcpApp {
  app: AgentApp;
  dispose(): Promise<void>;
}

async function createRuntime(workDir: string, sessionId?: string): Promise<AcpRuntime> {
...
function validateWorkDir(cwd: string): void {
  if (!isAbsolute(cwd)) {
    throw acp.RequestError.invalidParams(undefined, "Session cwd must be absolute.");
  }
}
...
export class SwiftyAcpAgent {
  private readonly sessions = new Map<string, AcpSession>();

  constructor(private readonly runtimeFactory: AcpRuntimeFactory = createRuntime) {}
  ...
  async loadSession(
    ...
    if (session && session.runtime.workDir !== params.cwd) {
      throw acp.RequestError.invalidParams(undefined, "Session cwd does not match.");
    }
    if (session && !replay) {
      return {};
    }
    ...
    if (!session && !existsSync(getSessionFilePath(params.cwd, params.sessionId))) {
    ...
    if (replay) {
      for (const notification of historyNotifications(params.sessionId, saved, params.cwd)) {
  ...
  listSessions(params: ListSessionsRequest): ListSessionsResponse {
    if (params.cursor) {
      throw acp.RequestError.invalidParams(undefined, "Session pagination is not supported.");
    }
  ...
  prompt(
    ...
    const run: Promise<PromptResponse> = session.tail
      .catch(() => undefined)
      .then(async () => {
        if (session.closed || generation !== session.generation || context.signal.aborted) {
        ...
        try {
          return await this.runPrompt(session, params.sessionId, text, context, signal);
        } finally {
  ...
  private async runPrompt(
    ...
    for await (const event of session.runtime.run(text, {
      onPermissionRequest: async (toolName, args, decision, toolCallId) => {
        const permission = context.client.request(acp.methods.client.session.requestPermission, {
          sessionId,
          toolCall: {
            toolCallId,
      ...
      if (update) {
        ...
        await context.client.notify(acp.methods.client.session.update, notification);
      }
    }
...
export function createSwiftyAcpApp(runtimeFactory?: AcpRuntimeFactory): SwiftyAcpApp {
  const implementation = new SwiftyAcpAgent(runtimeFactory);
  const app = acp
    .agent({ name: "swifty" })
    .onRequest(acp.methods.agent.initialize, (context) => implementation.initialize(context.params))
    .onRequest(acp.methods.agent.authenticate, () => implementation.authenticate())
