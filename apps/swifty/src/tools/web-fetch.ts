...
function cacheSet(url: string, markdown: string, finalUrl: string): void {
  ...
  urlCache.set(url, { markdown, finalUrl, expiresAt: Date.now() + CACHE_TTL_MS, size });
  urlCacheBytes += size;
...
function formatResult(markdown: string, requestedUrl: string, finalUrl: string): string {
...
export class WebFetchTool implements Tool {
  ...
  async execute(ctx: ToolContext, args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (cached) {
      return { output: formatResult(cached.markdown, url, cached.finalUrl), isError: false };
    }
    ...
    try {
      response = await fetch(url, {
        ...
        headers: {
          ...
          // "User-Agent": `swifty/${version}`,
        },
      });
    } catch (err) {
      log.error({ err, url }, "web fetch failed");
      return { output: `Error: fetch failed: ${asErrorString(err)}`, isError: true };
    }
