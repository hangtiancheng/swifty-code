...
function initialize(context: acp.ClientContext): Promise<acp.InitializeResponse> {
...
describe("Swifty ACP agent", () => {
  it("runs prompts, streams events and bridges permissions", async () => {
    const workDir = mkdtempSync(join(tmpdir(), "swifty-acp-"));
    ...
    const implementation = createSwiftyAcpApp(factory);
  ...
  it("cancels a turn waiting for a permission response", async () => {
    ...
    const implementation = createSwiftyAcpApp(() => Promise.resolve(runtime));
    const client = acp
      .client({ name: "test-client" })
      .onRequest(
  ...
  it("lists, loads and replays persisted sessions", async () => {
    ...
    saveMessage(workDir, sessionId, { role: "user", content: "hello", timestamp: 1 });
    ...
    try {
      await client.connectWith(implementation.app, async (context) => {
        await initialize(context);
        const listed = await context.request(acp.methods.agent.session.list, { cwd: workDir });
        expect(listed.sessions).toMatchObject([{ sessionId, cwd: workDir, title: "hello" }]);
